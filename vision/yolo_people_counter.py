...
class PeopleCounter:
    ...
    def detect_and_count(self, frame, current_camera=None, available_cameras=None):
        ...
        # Get detections
        detections = results[0].boxes if len(
            results) > 0 and results[0].boxes is not None else []
        ...
        for i, detection in enumerate(detections):
            if detection.conf > self.confidence_threshold and detection.cls == 0:
                ...
                # Draw label
                confidence_value = float(confidence.item()) if hasattr(
                    confidence, 'item') else float(confidence)
